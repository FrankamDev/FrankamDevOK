...
export default function LessonShow() {
  ...
  // 1. Gestion du Scroll (Barre de progression)
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Bonus : Détection simple du titre actif dans le sommaire
      const currentHeadings = headings.map(h => document.getElementById(h.id));
      const active = currentHeadings.find(el => el && el.getBoundingClientRect().top > 0);
  ...
  // 2. Traitement du contenu HTML (Titres & ID)
  useEffect(() => {
    if (!lesson.content) {
  ...
  // 3. Gestion des boutons "Copier" (Délégation d'événements)
  useEffect(() => {
    ...
    // Ajout des boutons à chaque bloc <pre>
    const blocks = currentContent.querySelectorAll('pre');
    blocks.forEach((block) => {
      if (block.querySelector('.copy-btn')) return;
      block.style.position = 'relative'; // Sécurité pour le placement du bouton
    ...
    const handleCopyClick = async (e: MouseEvent) => {
      ...
      // Nettoyage pour ne pas copier le texte "Copier" du bouton lui-même
      const cleanCode = code.replace(/Copier/g, '').trim();
      ...
      <div className="min-h-screen bg-[#030313] text-gray-100 selection:bg-indigo-500/30">
        ...
        <main className="lg:ml-80 min-h-screen flex flex-col">
          <div className="max-w-4xl mx-auto w-full px-5 sm:px-8 lg:px-12 py-12 lg:py-20">
            ...
            {/* Barre de scroll personnalisée si nécessaire */}
            <ScrollBar />
            ...
            {lesson.pdf_path && (
              <div className="my-12">
                <a href={lesson.pdf_path} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-indigo-500/50 transition-all group">
            ...
            <footer className="mt-20 pt-10 border-t border-white/5">
              <div className="bg-gradient-to-br from-[#0a0a20] to-[#030313] border border-white/5 p-8 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="text-center md:text-left">
                  <h4 className="text-xl font-bold mb-2 text-white">Prêt pour la suite ?</h4>
                  <p className="text-gray-400">Continuez vers le prochain chapitre pour valider vos acquis.</p>
                </div>
                <Link href={`/courses/${course.slug}`} className="bg-indigo-600 hover:bg-indigo-500 px-8 py-4 rounded-xl font-bold transition-all flex items-center gap-2 group text-white">
